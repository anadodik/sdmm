...
using TangentSpace =
    sdmm::SpatioDirectionalTangentSpace<JointEmbedded, JointTangent>;
...
static_assert(
    std::is_same_v<decltype(enoki::packet(Stats(), 0))::ScalarExpr, Packet>);
...
auto add_euclidian(py::module& m) {
    ...
    py::class_<TangentSpace>(m, name.c_str())
        ...
        .def(
            "set_mean",
            py::overload_cast<const Embedded&>(&TangentSpace::set_mean),
            "mean"_a)
        .def(
            "to_tangent",
            &TangentSpace::template to<EmbeddedS>,
            "embedded"_a,
            "inv_jacobian"_a)
        .def(
            "from_tangent",
            &TangentSpace::template from<TangentS>,
            "tangent"_a,
            "inv_jacobian"_a);
}
...
auto add_directional(py::module& m) {
    ...
    py::class_<TangentSpace>(m, name.c_str())
        ...
        .def(
            "get_from",
            [](TangentSpace& ts) { return ts.coordinate_system.from; })
        .def(
            "set_to",
            [](TangentSpace& ts, Matrix& m) { ts.coordinate_system.to = m; })
        .def(
            "set_from",
            [](TangentSpace& ts, Matrix& m) { ts.coordinate_system.from = m; })
        .def(
            "set_mean",
            py::overload_cast<const Embedded&>(&TangentSpace::set_mean),
            "mean"_a)
        ...
        .def(
            "from_jacobian",
            &TangentSpace::template from_jacobian<TangentS>,
            "tangent"_a)
        .def(
            "from_tangent_many",
            [](TangentSpace& ts,
               Tangent& tangent) -> std::pair<Embedded, Value> {
                if (enoki::slices(ts) != 1) {
                    throw std::runtime_error(
                        "tangent_space can only have one slice for from_tangent_many");
                }
...
auto add_mm_em(py::module& m, const std::string& name) {
    ...
    py::class_<EM>(m, name.c_str())
        ...
        .def("step", [](EM& em, SDMM& sdmm, sdmm::Data<SDMM>& data) {
...
auto add_mm(py::module& m, const std::string& name) {
    ...
    py::class_<SDMM>(m, name.c_str())
        ...
        .def(
            ...
            [](SDMM& sdmm,
               RNG& rng,
               size_t n_samples) -> std::pair<Embedded, Value> {
                EmbeddedS sample;
                TangentS tangent;
                Scalar inv_jacobian;
...
auto add_conditioner(py::module& m, const std::string& name) {
    ...
    py::class_<Conditioner>(m, name.c_str())
        ...
        .def(
            "prepare",
            [](Conditioner& conditioner, Joint& joint) {
        ...
        .def(
            "condition",
            [](Conditioner& conditioner, Point& point) {
        ...
        .def(
            ...
            [](Conditioner& conditioner,
               ...
               size_t preserve_idx = -1) {
                Conditional conditional;
                enoki::set_slices(conditional, enoki::slices(conditioner));
                sdmm::create_conditional_pruned(
                    conditioner,
                    point,
                    conditional,
                    max_components,
...
auto add_sdmm(py::module& dist_m, py::module& opt_m) {
    ...
    using MarginalTangentSpace =
        sdmm::EuclidianTangentSpace<MarginalEmbedded, MarginalTangent>;
    using MarginalSDMM =
    ...
    using ConditionalTangentSpace =
        sdmm::DirectionalTangentSpace<ConditionalEmbedded, ConditionalTangent>;
    ...
    py::class_<Accelerator>(dist_m, fmt::format("Accelerator{}", Size).c_str())
        ...
        .def(
            ...
            [](Accelerator& accelerator,
               sdmm::Vector<Scalar, 3>& point) -> JointSDMM {
                AABB aabb;
...
auto add_gmm(py::module& dist_m, py::module& opt_m) {
    ...
    using SDMM = sdmm::SDMM<sdmm::Matrix<Value, Size>, TangentSpace>;
...
auto add_dmm(py::module& dist_m, py::module& opt_m) {
    ...
    using SDMM = sdmm::SDMM<sdmm::Matrix<Value, 2>, TangentSpace>;
    ...
    py::class_<Accelerator>(dist_m, fmt::format("DMMAccelerator").c_str())
        ...
        .def(
            "find",
            [](Accelerator& accelerator,
               sdmm::Vector<Scalar, 3>& point) -> SDMM {
                AABB aabb;
...
PYBIND11_MODULE(pysdmm, m) {
    ...
    py::class_<RNG>(m, "RNG").def(py::init<>());
