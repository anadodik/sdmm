...
template <
    ...
    typename MatrixOut,
    std::enable_if_t<!TangentSpace::HasTangentSpaceOffset, int> = 0>
inline auto to_centered_tangent_space(
    ...
    MatrixOut& new_cov) -> void {
    using TangentExpr = enoki::expr_t<Tangent>;
    using MatrixExpr = enoki::expr_t<Matrix>;
    auto [new_mean, from_jacobian] =
        old_tangent_space.from_jacobian(new_mean_tangent);
...
inline auto to_centered_tangent_space(
    ...
    new_tangent_space.tangent_mean =
        old_tangent_space.tangent_mean + new_mean_tangent;
...
template <
    ...
    typename SDMM2,
    std::enable_if_t<
        !SDMM1::TangentSpace::IsEuclidian && !SDMM2::TangentSpace::IsEuclidian,
        int> = 0>
auto product(SDMM1& first, SDMM2& second, SDMM1& result) {
    ...
    using ScalarExpr =
        enoki::expr_t<decltype(enoki::packet(first.weight.pmf, 0))>;
    using TangentExpr = enoki::expr_t<typename decltype(
        enoki::packet(first.tangent_space, 0))::TangentExpr>;
    ...
    for (size_t first_i = 0; first_i < first_packets; ++first_i) {
        ...
        for (size_t second_i = 0; second_i < second_size; ++second_i) {
            ...
            auto [second_mean, to_jacobian] =
                first_ts.to_jacobian(second_mean_embedded, inv_jacobian);
            MatrixExpr J = to_jacobian * from_center_jacobian;

            // spdlog::info("J_to={}", J);
            MatrixExpr second_cov =
                J * second_cov_original * linalg::transpose(J);
            // spdlog::info("fc={}, first_cov={} vs fc0={}", first.cov,
            // first_cov, enoki::slice(first.cov, 0));
            ...
            if (!enoki::all(is_psd)) {
                ...
                auto embedded_local =
                    first_ts.coordinate_system.to * second_mean_embedded;
                std::cerr << fmt::format(
                    "({}, {}) = "
                    "is_psd={}, "
            ...
            MatrixExpr inv_cov_sum_sqrt =
                linalg::inverse_lower_tri(cov_sum_sqrt);
            ...
            MatrixExpr new_cov_tangent =
                linalg::transpose(first_cov_premult) * second_cov_premult;
            auto new_mean_tangent = linalg::transpose(second_cov_premult) *
                    first_mean_premult + // == 0
            ...
            ScalarExpr new_weight = first_weight * second_weight *
                inv_cov_sqrt_det * inv_jacobian *
...
auto product(SDMM1& first, SDMM2& second, SDMM1& result) {
    ...
    for (size_t first_i = 0; first_i < first_size; ++first_i) {
        for (size_t second_i = 0; second_i < second_size; ++second_i) {
            ...
            EmbeddedS first_mean =
                enoki::slice(first.tangent_space.mean, first_i);
            EmbeddedS second_mean =
                enoki::slice(second.tangent_space.mean, second_i);
