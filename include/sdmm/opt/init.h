...
void initialize(
    ...
    Value min_spatial_distances =
        enoki::full<Value>(std::numeric_limits<float>::infinity(), data.size);
    Value min_normal_distances =
        enoki::full<Value>(std::numeric_limits<float>::infinity(), data.size);
    ...
    for (size_t sc_i = 0; sc_i < n_spatial_components; ++sc_i) {
        // size_t point_i = enoki::min(rng.next_float32() * data.size, data.size
        // - 1);
        ...
        Matrix3 manifold_cov = width_var * linalg::outer(s) +
            width_var * linalg::outer(t) + depth_var * linalg::outer(n);

        Matrix3 depth_prior = linalg::outer(n) * 1e-6;

        // spdlog::info("n={}, s={}, t={}, manifold_cov={}", n, s, t,
        // manifold_cov);
        for (size_t sc_j = 0; sc_j < data.size; ++sc_j) {
            ...
            Vector3 position_j(
                mean_j.coeff(0), mean_j.coeff(1), mean_j.coeff(2));
            ...
            if (d < min_distances.coeff(sc_j)) {
                ...
                ScalarS metric = enoki::min(
                    enoki::max(enoki::slice(data.weight, sc_j), 1e-3), 3);
                enoki::slice(sampling_dist.pmf, sc_j) =
                    metric * enoki::pow(d, 5);
            }

            if (normal_d < NORMAL_THRESHOLD &&
                position_d < min_spatial_distances.coeff(sc_j)) {
                min_spatial_distances.coeff(sc_j) = position_d;
                min_normal_distances.coeff(sc_j) = normal_d;
            }

            if (min_normal_distances.coeff(sc_j) < NORMAL_THRESHOLD &&
        ...
        if (!sampling_dist.prepare()) {
            ...
            sampling_dist.pmf = enoki::full<Value>(
                1.f / enoki::slices(sampling_dist.pmf),
                enoki::slices(data.weight));
            ...
            for (size_t phi_i = 0; phi_i < n_phis; ++phi_i) {
                ...
                for (size_t r = 0; r < 3; ++r) {
                    for (size_t c = 0; c < 3; ++c) {
                        enoki::slice(em.depth_prior, component_i)(r, c) =
                            depth_prior(r, c);
                        enoki::slice(sdmm.cov, component_i)(r, c) =
                            manifold_cov(r, c);
                    }
                }
                // spdlog::info("n={}, s={}, t={}, cov=\n{}", n, s, t,
                // enoki::slice(sdmm.cov, component_i));
                ++component_i;
            }
        }
    }
...
template <typename DMM, typename EM, typename Data, typename RNG>
void initialize_dmm(
    DMM& dmm,
    EM& em,
    Data& data,
    ...
    size_t n_spatial_components,
    float spatial_distance) {
    using EmbeddedS = sdmm::embedded_s_t<DMM>;
    using NormalS = sdmm::normal_s_t<Data>;
    using ScalarS = typename DMM::ScalarS;
    using Vector3 = sdmm::Vector<float, 3>;
    using CoordinateSystem = linalg::CoordinateSystem<Vector3>;
    using Matrix3 = typename CoordinateSystem::Rotation;
    using Value = decltype(data.weight);
    ...
    ScalarS n_phis = 4;
    ScalarS n_components = n_spatial_components * n_thetas * n_phis;
    enoki::set_slices(dmm, (size_t)n_components);
    em = enoki::zero<EM>((size_t)n_components);

    dmm.cov = enoki::zero<decltype(dmm.cov)>(n_components);
    static const boost::math::chi_squared chi_sqr(3);
    static const ScalarS contained_mass = 0.90;
    static const ScalarS max_rad_sqr =
        (ScalarS)boost::math::quantile(chi_sqr, contained_mass);
    ...
    ScalarS directional_var = 2.f * M_PI / 8.f;
    dmm.cov(0, 0) = enoki::full<Value>(directional_var, (size_t)n_components);
    dmm.cov(1, 1) = enoki::full<Value>(directional_var, (size_t)n_components);
    ...
    for (size_t sc_i = 0; sc_i < n_spatial_components; ++sc_i) {
        ...
        for (size_t sc_j = 0; sc_j < data.size; ++sc_j) {
            ...
            if (normal_d < NORMAL_THRESHOLD &&
                normal_d < min_normal_distances.coeff(sc_j)) {
                min_normal_distances.coeff(sc_j) = normal_d;
            }
        }
        if (!sampling_dist.prepare()) {
        ...
        for (size_t theta_i = 0; theta_i < n_thetas; ++theta_i) {
            ...
            for (size_t phi_i = 0; phi_i < n_phis; ++phi_i) {
                ...
                sdmm::Vector<float, 3> direction(
                    sin_theta * cos_phi, sin_theta * sin_phi, cos_theta);
                direction = coordinates.from * direction;
                enoki::slice(dmm.tangent_space, component_i)
                    .set_mean(direction);
                ++component_i;
            }
        }
    ...
    dmm.weight.pmf =
        enoki::full<Value>(1.f / n_components, (size_t)n_components);
    sdmm::prepare_vectorized(dmm);

    ScalarS weight_prior = 1.f / n_components;
    ScalarS cov_prior_strength = 5.f / n_components;
    sdmm::matrix_t<DMM> cov_prior =
        enoki::zero<sdmm::matrix_t<DMM>>((size_t)n_components);
    for (size_t slice_i = 0; slice_i < n_components; ++slice_i) {
        enoki::slice(cov_prior, slice_i) =
            sdmm::matrix_s_t<DMM>(2e-4, 0, 0, 2e-4);
    }
    em.set_priors(weight_prior, cov_prior_strength, cov_prior);
}
