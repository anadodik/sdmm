...
struct DirectionalTangentSpace {
    ...
    auto from(const TangentIn& tangent, ScalarExpr& inv_jacobian) const
        ...
        const EmbeddedExpr embedded_local{
            tangent.x() * sinc_angle, tangent.y() * sinc_angle, cos_angle};
    ...
    auto from_jacobian(const TangentIn& tangent) const
        ...
        jacobian(0, 0) = sinc_angle +
            tangent.coeff(0) * tangent.coeff(0) *
                cos_minus_sinc_over_length_sqr;
        jacobian(1, 1) = sinc_angle +
            tangent.coeff(1) * tangent.coeff(1) *
                cos_minus_sinc_over_length_sqr;

        ScalarExpr off_diagonal = tangent.coeff(0) * tangent.coeff(1) *
            cos_minus_sinc_over_length_sqr;
        ...
        jacobian(2, 0) = enoki::select(
            sin_angle < 1e-2 || cos_angle < cos_angle_min,
            0,
            -tangent.coeff(0) * sinc_angle);
        jacobian(2, 1) = enoki::select(
            sin_angle < 1e-2 || cos_angle < cos_angle_min,
            0,
            -tangent.coeff(1) * sinc_angle);
...
void to_json(
    json& j,
    const DirectionalTangentSpace<Embedded, Tangent>& tangent_space) {
    j = json{
...
void from_json(
    const json& j,
    DirectionalTangentSpace<Embedded, Tangent>& tangent_space) {
