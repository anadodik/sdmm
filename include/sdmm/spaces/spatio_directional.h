...
struct SpatioDirectionalTangentSpace {
    ...
    static_assert(
        Embedded_::Size >= 3 && Tangent_::Size == Embedded_::Size - 1);
    ...
    auto to_unwrap(
        ...
        const ScalarExpr& directional2) const -> TangentExpr {
        return {
    ...
    auto from_unwrap(
        ...
        const ScalarExpr& directional3) const -> EmbeddedExpr {
        return EmbeddedExpr(
    ...
    template <
        typename EmbeddedIn,
        std::enable_if_t<EmbeddedIn::Size == Embedded::Size, int> = 0>
    auto directional(const EmbeddedIn& embedded) const
        -> DirectionalEmbeddedExpr {
        return {
    ...
    template <
        typename TangentIn,
        std::enable_if_t<TangentIn::Size == Tangent::Size, int> = 0>
    auto directional(const TangentIn& tangent) const -> DirectionalTangentExpr {
        return {
            tangent.coeff(Tangent::Size - 2), tangent.coeff(Tangent::Size - 1)};
    }
    ...
    auto from(const TangentIn& tangent, ScalarExpr& inv_jacobian) const
        ...
        const ScalarExpr sinc_angle =
            enoki::select(sin_angle < 1e-4, ScalarExpr(1), sin_angle / length);
        ...
        const DirectionalEmbeddedExpr embedded_local{
            ...
            cos_angle};
        const DirectionalEmbeddedExpr embedded =
            coordinate_system.from * embedded_local;
    ...
    auto to_center_jacobian() const
        -> sdmm::Matrix<ScalarExpr, Tangent::Size, Embedded::Size> {
        using DirectionalJacobian = sdmm::Matrix<ScalarExpr, 2, 3>;
        using Jacobian =
            sdmm::Matrix<ScalarExpr, Tangent::Size, Embedded::Size>;
        DirectionalJacobian directional_jacobian =
            enoki::zero<DirectionalJacobian>();
        ...
        for (size_t diag_i = 0; diag_i < Tangent::Size - 2; ++diag_i) {
            jacobian(diag_i, diag_i) =
                enoki::full<ScalarExpr>(1.f, enoki::slices(jacobian));
        }
        for (size_t r = 0; r < 2; ++r) {
            for (size_t c = 0; c < 3; ++c) {
                jacobian(r + Tangent::Size - 2, c + Embedded::Size - 3) =
                    directional_jacobian(r, c);
            }
        }
        return jacobian;
    }
    ...
    // \log_{\mu} ( \vec{t} )
    template <typename TangentIn>
    auto from_jacobian(const TangentIn& tangent) const -> std::pair<
        EmbeddedExpr,
        sdmm::Matrix<ScalarExpr, Embedded::Size, Tangent::Size>> {
        ...
        directional_jacobian(0, 0) = sinc_angle +
            directional_tangent.coeff(0) * directional_tangent.coeff(0) *
                cos_minus_sinc_over_length_sqr;
        directional_jacobian(1, 1) = sinc_angle +
            directional_tangent.coeff(1) * directional_tangent.coeff(1) *
                cos_minus_sinc_over_length_sqr;

        ScalarExpr off_diagonal = directional_tangent.coeff(0) *
            directional_tangent.coeff(1) * cos_minus_sinc_over_length_sqr;
        ...
        directional_jacobian(2, 0) = enoki::select(
            sin_angle < 1e-2 || cos_angle < cos_angle_min,
            0,
            -directional_tangent.coeff(0) * sinc_angle);
        directional_jacobian(2, 1) = enoki::select(
            sin_angle < 1e-2 || cos_angle < cos_angle_min,
            0,
            -directional_tangent.coeff(1) * sinc_angle);
        ...
        for (size_t r = 0; r < 3; ++r) {
            for (size_t c = 0; c < 2; ++c) {
                jacobian(r + Embedded::Size - 3, c + Tangent::Size - 2) =
                    directional_jacobian(r, c);
            }
        }
        ...
        const DirectionalEmbeddedExpr embedded_local{
            ...
            directional_tangent.y() * sinc_angle,
            cos_angle};
        const DirectionalEmbeddedExpr directional_embedded =
            coordinate_system.from * embedded_local;
...
void to_json(
    json& j,
    const SpatioDirectionalTangentSpace<Embedded, Tangent>& tangent_space) {
    j = json{
...
void from_json(
    const json& j,
    SpatioDirectionalTangentSpace<Embedded, Tangent>& tangent_space) {
