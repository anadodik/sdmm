...
struct Matrix : enoki::StaticArrayImpl<
                    enoki::Array<Value_, Rows_>,
                    Cols_,
                    false,
                    Matrix<Value_, Rows_, Cols_>> {
    ...
    using Base = enoki::
        StaticArrayImpl<Column, Cols_, false, Matrix<Value_, Rows_, Cols_>>;
    using Base::coeff;
    ...
    template <typename T>
    using ReplaceValue = Matrix<enoki::value_t<T>, Rows_, Cols_>;
    template <size_t R, size_t C>
    using ReplaceSize = Matrix<enoki::value_t<Value>, R, C>;
    ...
    ENOKI_INLINE Matrix(const Matrix<Value2, Rows2, Cols2>& m) : Base(m) {}

    template <
        typename T,
        enoki::enable_if_t<(enoki::array_depth_v<T> <= Base::Depth - 2)> = 0,
    ...
    template <
        typename T,
        enoki::enable_if_t<(enoki::array_depth_v<T> == Base::Depth)> = 0,
        enoki::enable_if_not_matrix_t<T> = 0>
    ...
    ENOKI_INLINE decltype(auto) operator()(size_t i, size_t j) {
        return coeff(j, i);
    }
    ENOKI_INLINE decltype(auto) operator()(size_t i, size_t j) const {
        return coeff(j, i);
    }

    ENOKI_INLINE Column& col(size_t index) {
        return coeff(index);
    }
    ENOKI_INLINE const Column& col(size_t index) const {
        return coeff(index);
    }
};
...
ENOKI_INLINE Result operator*(
    const Matrix<T0, Rows1, Cols1>& m0,
...
template <
    ...
    size_t Rows,
    size_t Cols,
    enoki::enable_if_t<!T1::IsMatrix> = 0>
ENOKI_INLINE auto operator*(
    const sdmm::linalg::Matrix<T0, Rows, Cols>& m,
    const T1& s) {
    if constexpr (
        enoki::array_size_v<T1> == Cols && T1::IsVector &&
        !std::is_same_v<T1, T0>) {
        ...
        //     sum = enoki::fmadd(m.coeff(i), EVector::full_(s.coeff(i), 1),
        //     sum);
        // }
        // return sum;
    } else {
...
ENOKI_INLINE T identity(size_t size = 1) {
    ...
    for (size_t i = 0; i < T::Cols; ++i)
        result(i, i) =
            enoki::full<typename T::Entry>(enoki::scalar_t<T>(1.f), size);
    return result;
}
...
struct struct_support<
    sdmm::linalg::Matrix<T, Rows, Cols>,
   ...
   private:
    ...
    static ENOKI_INLINE auto
    packet(T2&& value, size_t i, std::index_sequence<Index...>) {
        return sdmm::linalg::
            Matrix<decltype(enoki::packet(value.coeff(0, 0), i)), Rows, Cols>(
                enoki::packet(value.coeff(Index), i)...);
    }

    template <typename T2, size_t... Index>
    static ENOKI_INLINE auto
    slice(T2&& value, size_t i, std::index_sequence<Index...>) {
        return sdmm::linalg::
            Matrix<decltype(enoki::slice(value.coeff(0, 0), i)), Rows, Cols>(
                enoki::slice(value.coeff(Index), i)...);
    }

    template <typename T2, size_t... Index>
    static ENOKI_INLINE auto
    slice_ptr(T2&& value, size_t i, std::index_sequence<Index...>) {
        return sdmm::linalg::Matrix<
            decltype(enoki::slice_ptr(value.coeff(0, 0), i)),
            Rows,
            Cols>(enoki::slice_ptr(value.coeff(Index), i)...);
    }

    template <typename T2, size_t... Index>
    static ENOKI_INLINE auto ref_wrap(
        T2&& value,
        std::index_sequence<Index...>) {
        return sdmm::linalg::
            Matrix<decltype(enoki::ref_wrap(value.coeff(0, 0))), Rows, Cols>(
    ...
    static ENOKI_INLINE auto detach(T2&& value, std::index_sequence<Index...>) {
        return sdmm::linalg::
            Matrix<decltype(enoki::detach(value.coeff(0, 0))), Rows, Cols>(
                enoki::detach(value.coeff(Index))...);
    }

    template <typename T2, size_t... Index>
    static ENOKI_INLINE auto gradient(
        T2&& value,
        std::index_sequence<Index...>) {
        return sdmm::linalg::
            Matrix<decltype(enoki::gradient(value.coeff(0, 0))), Rows, Cols>(
