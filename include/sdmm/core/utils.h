...
#define VECTORIZE_WRAP_FWD(FUNC_NAME)                         \
    [](auto&&... params) {                                    \
        FUNC_NAME(std::forward<decltype(params)>(params)...); \
    }
#define VECTORIZE_WRAP_OUTPUT(FUNC_NAME) \
    [](auto&& output, auto&&... params) { output = FUNC_NAME(params...); }
...
struct nested_packet_size<
    Value,
    std::enable_if_t<!enoki::is_dynamic_v<Value>>> {
...
struct outer_type<
    ...
    using type =
        std::remove_reference_t<typename Value::template ReplaceValue<New>>;
};
...
template <typename Value, typename New>
using outer_type_t =
    typename outer_type<std::remove_reference_t<Value>, New>::type;
...
struct MutexWrapper {
    ...
    MutexWrapper& operator=(
        [[maybe_unused]] const MutexWrapper& mutex_wrapper) {
        return *this;
    };
    MutexWrapper& operator=([[maybe_unused]] MutexWrapper&& mutex_wrapper) {
        return *this;
    };
